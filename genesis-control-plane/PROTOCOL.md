...
- Stable Work dispatcher inbox alias: `work:genesis-dispatcher`
...
2. Determine the current Work chat identity, then check BOTH its addressed GENESIS Live Bus inbox and the stable dispatcher inbox `work:genesis-dispatcher`. The dispatcher alias exists specifically because the concrete Work chat identity may change between wakes.
...
6. For an addressed `RESEARCH_VERIFICATION` dispatcher mission from either inbox, process at most 3 embedded sub-missions in the payload. Keep evidence tracks separate. Use public web research and authorized read-only project context only. Do not modify Bridge, phone, production runtime, database schema/grants, or project truth state.
...
## Pilot
The pilot branch is isolated from main. First test: GitHub write/read. Second test: phone -> GitHub. Third test: GitHub PR activity -> ChatGPT Work wake trigger. Fourth test: stable dispatcher inbox -> bounded research batch -> RESULT/BLOCK.
